            ...
            <div className="flex items-center space-x-3">
                ...
                <div>
                    ...
                    <p className="text-sm text-slate-600">
                        {user ? `Bienvenue, ${user.name || 'futur papa'} !` : 'Le guide pour futurs et nouveaux pères.'}
                    </p>
                </div>
            </div>
            <div>
                {user ? (
                    <button onClick={onLogoutClick} className="bg-red-600 text-white px-4 py-2 rounded-lg font-semibold hover:bg-red-500 transition-colors">Déconnexion</button>
                ) : (
                    <button onClick={onLoginClick} className="bg-slate-800 text-white px-4 py-2 rounded-lg font-semibold hover:bg-slate-700 transition-colors">Connexion</button>
                )}
            </div>
        </div>
    </header>
    ...
    const handleAuth = async (e, authAction) => {
        ...
        try {
            if (authAction === 'signup') {
                ...
                await firestoreService.saveUserData(userCredential.user.uid, { name, email, createdAt: new Date() });
            ...
            <div className="bg-white rounded-xl shadow-2xl p-8 w-full max-w-md relative" onClick={e => e.stopPropagation()}>
                ...
                {isLoginView ? (
                    <div>
                        ...
                        <p className="text-center text-slate-600 mb-6">Connectez-vous pour retrouver votre parcours.</p>
                        <form onSubmit={(e) => handleAuth(e, 'login')}>
                            <div className="mb-4">
                                <label htmlFor="login-email" className="block text-sm font-medium text-slate-700 mb-1">Email</label>
                                <input type="email" id="login-email" name="email" required className="w-full px-3 py-2 border border-slate-300 rounded-md focus:outline-none focus:ring-2 focus:ring-slate-500" />
                            </div>
                            <div className="mb-6">
                                <label htmlFor="login-password">Mot de passe</label>
                                <input type="password" id="login-password" name="password" required className="w-full px-3 py-2 border border-slate-300 rounded-md focus:outline-none focus:ring-2 focus:ring-slate-500" />
                            </div>
                            <button type="submit" className="w-full bg-slate-800 text-white py-3 rounded-lg font-semibold hover:bg-slate-700 transition-colors">Connexion</button>
                        </form>
                        <p className="text-center text-sm mt-4">Pas de compte ? <button onClick={() => setIsLoginView(false)} className="font-semibold text-slate-600 hover:underline">Inscrivez-vous</button></p>
                    </div>
                ) : (
                    <div>
                        <h2 className="text-2xl font-bold text-center mb-1">Créez votre espace</h2>
                        <p className="text-center text-slate-600 mb-6">Rejoignez-nous pour commencer l'aventure.</p>
                        <form onSubmit={(e) => handleAuth(e, 'signup')}>
                            <div className="mb-4">
                                <label htmlFor="signup-name">Votre prénom</label>
                                <input type="text" id="signup-name" name="name" required className="w-full px-3 py-2 border border-slate-300 rounded-md focus:outline-none focus:ring-2 focus:ring-slate-500" />
                            </div>
                            <div className="mb-4">
                                <label htmlFor="signup-email">Email</label>
                                <input type="email" id="signup-email" name="email" required className="w-full px-3 py-2 border border-slate-300 rounded-md focus:outline-none focus:ring-2 focus:ring-slate-500" />
                            </div>
                            <div className="mb-6">
                                <label htmlFor="signup-password">Mot de passe</label>
                                <input type="password" id="signup-password" name="password" required className="w-full px-3 py-2 border border-slate-300 rounded-md focus:outline-none focus:ring-2 focus:ring-slate-500" />
                            </div>
                            <button type="submit" className="w-full bg-slate-800 text-white py-3 rounded-lg font-semibold hover:bg-slate-700 transition-colors">Créer mon compte</button>
                        </form>
                        <p className="text-center text-sm mt-4">Déjà un compte ? <button onClick={() => setIsLoginView(true)} className="font-semibold text-slate-600 hover:underline">Connectez-vous</button></p>
                    </div>
                )}
            </div>
        </div>
...
const Toast = ({ message, type, isVisible }) => {
    ...
    return (
        <div className={`fixed bottom-5 right-5 text-white px-6 py-3 rounded-lg shadow-lg transition-opacity duration-300 ${isVisible ? 'opacity-100' : 'opacity-0'} ${bgColor}`}>
            <p>{message}</p>
        </div>
    );
};
...
const LoggedOutView = ({ onLoginClick }) => (
    <div className="text-center py-16 bg-white rounded-xl shadow-lg">
        ...
        <p className="text-slate-600 mt-2 max-w-xl mx-auto">Connectez-vous ou créez un compte pour suivre votre parcours, obtenir des conseils culinaires et bien plus.</p>
        <button onClick={onLoginClick} className="mt-6 bg-slate-800 text-white px-6 py-3 rounded-lg font-semibold hover:bg-slate-700 transition-colors">Commencer mon parcours</button>
    </div>
);

const CuisineView = () => (
    <div className="bg-white p-6 md:p-8 rounded-xl shadow-lg animate-fade-in">
        <h2 className="text-3xl font-bold text-slate-900 mb-2 text-center">Côté Cuisine : Le Guide du Partenaire</h2>
        <p className="text-slate-600 text-center mb-8 max-w-2xl mx-auto">Pendant la grossesse, l'alimentation est capitale. En prenant les rênes de la cuisine, vous soutenez activement votre compagne et votre futur bébé. C'est un acte d'amour au quotidien.</p>
        <div className="space-y-8">
            <div className="border border-slate-200 rounded-lg p-6">
                <h3 className="text-2xl font-bold mb-4">1er Trimestre : Lutter contre la fatigue et les nausées</h3>
                <div className="grid md:grid-cols-2 gap-6">
                    <div>
                        <h4 className="font-semibold text-lg mb-2">Aliments à privilégier :</h4>
                        <ul className="list-disc list-inside space-y-1 text-slate-700"><li>Gingembre & Citron</li><li>Légumes à feuilles vertes (riches en B9)</li><li>Lentilles, pois chiches</li></ul>
                    </div>
                    <div>
                        <h4 className="font-semibold text-lg mb-2 text-red-600">Points de vigilance :</h4>
                        <ul className="list-disc list-inside space-y-1 text-red-500 text-sm"><li>Zéro alcool.</li><li>Toxoplasmose : Viande bien cuite, laver fruits/légumes.</li><li>Listériose : Éviter fromages au lait cru, charcuteries...</li></ul>
                    </div>
                </div>
            </div>
            <div className="border border-slate-200 rounded-lg p-6">
                <h3 className="text-2xl font-bold mb-4">2ème Trimestre : Construire les réserves</h3>
                 <div className="grid md:grid-cols-2 gap-6">
                    <div>
                        <h4 className="font-semibold text-lg mb-2">Aliments à privilégier :</h4>
                        <ul className="list-disc list-inside space-y-1 text-slate-700"><li>Fer : Viande rouge, lentilles...</li><li>Calcium : Produits laitiers, amandes...</li><li>Oméga-3 : Petits poissons gras, noix...</li></ul>
                    </div>
                    <div>
                        <h4 className="font-semibold text-lg mb-2">Idée Recette Facile :</h4>
                        <p className="font-bold">Dahl de Lentilles Corail</p><p className="text-sm text-slate-600">Plat complet, réconfortant et plein de fer.</p>
                    </div>
                </div>
            </div>
            <div className="border border-slate-200 rounded-lg p-6">
                <h3 className="text-2xl font-bold mb-4">3ème Trimestre & Post-Partum : Faire le plein d'énergie</h3>
                <div className="grid md:grid-cols-2 gap-6">
                    <div>
                        <h4 className="font-semibold text-lg mb-2">Aliments à privilégier :</h4>
                        <ul className="list-disc list-inside space-y-1 text-slate-700"><li>Sucres lents : Pâtes complètes, quinoa...</li><li>Magnésium : Chocolat noir, bananes...</li></ul>
                    </div>
                    <div>
                        <h4 className="font-semibold text-lg mb-2 text-blue-600">Action Clé : Le Batch Cooking du Papa !</h4>
                        <p className="text-sm text-slate-600">Un mois avant le terme, préparez et congelez des plats. Votre "vous du futur" vous remerciera !</p>
                    </div>
                </div>
            </div>
        </div>
            ...
            <div className="bg-white p-4 rounded-xl shadow-lg">
                ...
                <div className="flex items-start pb-4 space-x-4 overflow-x-auto">
                    {timelineData.map(item => (
                        <div key={item.id} onClick={() => setCurrentStageId(item.id)}
                            ...
                            <i className={`ph-bold ${item.icon} text-4xl mb-2 transition-colors ${currentStageId === item.id ? 'text-white' : 'text-slate-600'}`}></i>
            ...
            <div className="mt-8">
                <div className="bg-white p-6 rounded-xl shadow-lg">
                    ...
                    <div className="mt-6 border-t pt-6">
                        <h4 className="font-semibold text-lg mb-2 flex items-center"><i className="ph-bold ph-notebook mr-2"></i>Mes notes personnelles</h4>
                        <p className="text-sm text-slate-500 mb-3">Cet espace est le vôtre. Notez vos pensées, questions, ou moments importants.</p>
                        {isLoadingNote ? <div className="w-full h-32 bg-slate-200 rounded-md animate-pulse"></div> : (
                        ...
                        <button onClick={handleSaveNote} disabled={isLoadingNote} className="mt-3 bg-green-600 text-white px-5 py-2 rounded-lg font-semibold hover:bg-green-500 transition-colors disabled:bg-slate-400">Enregistrer mes notes</button>
                    </div>
                </div>
            </div>
        </div>
...
export default function App() {
    ...
    if (!authReady) {
        return <div className="flex items-center justify-center h-screen bg-slate-50 text-slate-600">Chargement de l'application...</div>;
    }
                ...
                {!user ? (
                    ...
                    <div>
                        <div className="mb-8 flex justify-center bg-slate-200 rounded-lg p-1 max-w-sm mx-auto">
                            <button onClick={() => setCurrentView('parcours')} className={`w-1/2 px-4 py-2 font-semibold rounded-md flex items-center justify-center space-x-2 transition-colors ${currentView === 'parcours' ? 'bg-slate-900 text-white' : ''}`}>
                                <i className="ph-bold ph-steps"></i><span>Mon Parcours</span>
                            </button>
                            <button onClick={() => setCurrentView('cuisine')} className={`w-1/2 px-4 py-2 font-semibold rounded-md flex items-center justify-center space-x-2 transition-colors ${currentView === 'cuisine' ? 'bg-slate-900 text-white' : ''}`}>
                                <i className="ph-bold ph-cooking-pot"></i><span>Côté Cuisine</span>
                            </button>
                        </div>
                        {currentView === 'parcours' ? <ParcoursView userId={user.uid} showToast={showToast} /> : <CuisineView />}
                    </div>
                )}
            </main>
